...
Real GKR::Qdot_GG()
{
  ...
  Real ic3a,ic3b,ic3c,ic4a,ic4b,ic4c,ic3bit,idtcorrection;
  ...
  ic3b=-22.24163077;
  ic3c=7.55265467;
  ic3bit=ic3a+ic3b/p+ic3c/(p*sqrt(p));
  idtcorrection=kerr_a*kerr_a*(247.1682656/sqrt(p)-162.2684644+csqi*(-267.5529723/sqrt(p)+184.4645976)
			       +kerr_a*(-182.165263315/sqrt(p)+152.125216225+csqi*(254.0668915/sqrt(p)
			       -188.131613584)))/(p*p);*/

  /* idot fits constrained by Sago et al. results */
  /* Option 1 */
  /*ic3a=-13./96.;
  ic3b=0.0849646556;
  ic3c=18.217143955;
  ic3bit=ic3a+ic3b/sqrt(p)+ic3c/p;
  idtcorrection=kerr_a*kerr_a*(305.608335617/sqrt(p)-187.19466346+csqi*(-267.55251597/sqrt(p)+184.464386098)
			       +kerr_a*(-224.940867296/sqrt(p)+170.370112255+csqi*(254.06646091/sqrt(p)
										   -188.13141316)))/(p*p);*/
  /* Option 2 */
  ic3a=-13./96.;
  ic3b=-18.979368894568502;
  ic3c=1.351708902618378;
  ic3bit=ic3a+ic3b/p+ic3c/(p*sqrt(p));
  idtcorrection=kerr_a*kerr_a*(296.122534572/sqrt(p)-183.547448796+csqi*(-267.552627718/sqrt(p)+184.4644399081)
			       +kerr_a*(-217.997679349/sqrt(p)+167.7005143617+csqi*(254.06656746319/sqrt(p)
			       -188.13146467772)))/(p*p);
  ...
  circpiece-=kerr_a*kerr_a*(ic3bit+kerr_a*cosiota*(ic4a/(sqrt(p))+ic4b/p+ic4c/(p*sqrt(p)))
			    +idtcorrection)/(p*p);
  ...
  dQdt=-12.8*Lovercosi*siniotasqr*(1.-e*e)*sqrt(1.-e*e)*
    (circpiece+7.*e*e/8.-(e*e*425./336.)/p+M_PI*(97.*e*e/8.)/(p*sqrt(p))
     ...
     +kerr_a*kerr_a*(847.*e*e/96.)/(p*p)+Lzesqcorr)/pow(p,3.5);
  /*dQdt=-12.8*Lovercosi*siniotasqr*(1.-e*e)*sqrt(1.-e*e)*
    (circpiece+7.*e*e/8.-(e*e*425./336.)/p+M_PI*(97.*e*e/8.)/(p*sqrt(p))
    -(302893.*e*e/6048.)/(p*p)-kerr_a*cosiota*e*e*(91./4.+461.*e*e/64.)/(p*sqrt(p))
    +kerr_a*kerr_a*(847.*e*e/96.-siniotasqr*37.*e*e/2.)/(p*p)+Lzesqcorr)/pow(p,3.5);*/
  return (dQdt);
}
...
Real GKR::Edot_GG()
{
  ...
  return(pref*(term1 - kerr_a*cosiota*term2/(pow(p, 1.5))-term3/p+M_PI*term4/pow(p,1.5)
	       -term5/(p*p)+kerr_a*kerr_a*(term6-siniotasqr*term7)/(p*p)+esqcorr)+fact*circbit);
}
...
Real GKR::Lzdot_GG()
{
  ...
  return(pref*(cosiota*term1 + kerr_a/(pow(p, 1.5))*(term2 - csqi*term3) -  
	       ...
	       kerr_a*kerr_a*(term7-siniotasqr*term8)*cosiota/(p*p) +esqcorr + circbit ));
}
